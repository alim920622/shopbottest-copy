...
from app.api.schemas import LogoutRequest, RefreshTokenRequest, TelegramAuthRequest, TokenResponse
...
async def _issue_tokens(db: Database, telegram_user_id: int) -> TokenResponse:
    ...
    async with db.conn() as conn:
        ...
        await conn.execute(
            ...
            VALUES (?, ?, ?, 0)
            """,
            (telegram_user_id, refresh_token, expires_at.isoformat()),
        )
        await conn.commit()
